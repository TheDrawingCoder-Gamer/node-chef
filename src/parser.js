...
const AnyNotNewline = createToken({
  name: "AnyNotNewline",
  pattern: /.+/,
  categories: Any,
});
...
const OrdinalSuffix = createToken({
  name: "OrdinalSuffix",
  pattern: /(th|st|rd|nd)/,
  categories: Any,
});
const Measure = createToken({
  name: "Measure",
...
const Unit = createToken({
  name: "Unit",
  pattern: /(k?g|pinch(es)?|m?l|dash(es)?|cups?|teaspoons?|tablespoons?])/,
  longer_alt: Identifier,
  categories: [Any, NamePart],
});
...
const Gas = createToken({
  name: "Gas",
  pattern: /gas/,
  categories: [Any, NamePart],
});
const Mark = createToken({
  name: "Mark",
  pattern: /mark/,
  categories: [Any, NamePart],
});
  ...
  constructor() {
    ...
    this.RULE("endLoop", () => {
      ...
      this.OPTION(() => {
        ...
        this.AT_LEAST_ONE(() =>
          this.CONSUME1(Identifier, { LABEL: "decName" })
        );
      });
      this.CONSUME(Until);
